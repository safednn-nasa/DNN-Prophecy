...
def generate_cub_concept_dataset(concept_id: int, subset_size: int, random_seed: int,
    ...
    dataset = CUBDataset(pkl_paths, use_attr, no_img, uncertain_label, image_dir, n_class_attr, transform)
    if concept_id == -1:
        # random select indexs
        np.random.seed(random_seed)
        positive_idx = np.random.choice(len(dataset), subset_size, replace=False)
        # exclude the positive indexs and select negative indexs
        negative_idx = np.setdiff1d(np.arange(len(dataset)), positive_idx)
        negative_idx = np.random.choice(negative_idx, subset_size, replace=False)
    else:
