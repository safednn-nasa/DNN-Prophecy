...
def plot_concept_disentanglement(results_dir: Path, dataset_name: str, train: bool, fig_size: float = 5) -> None:
    sns.set(font_scale=1)
    fig, ax = plt.subplots(figsize=(fig_size, fig_size))
    sns.set_theme(style="whitegrid")
    if train:
        metrics_df = pd.read_csv(results_dir/"metrics_train.csv", index_col=0)
    else:
        metrics_df = pd.read_csv(results_dir/"metrics_test.csv", index_col=0)
    sns.lineplot(data=metrics_df, ax=ax)
    plt.xlabel("Layer")
    plt.ylabel("Disentanglement")
    if train:
        plt.savefig(results_dir/f"{dataset_name}_disentanglement_train.pdf")
    else:
        plt.savefig(results_dir/f"{dataset_name}_disentanglement_test.pdf")
    plt.close()
