...
class YOLOLayer(nn.Module):
    ...
    def forward(self, p, out):
        ...
        if self.training:
            ...
            io = p.sigmoid()
            io_clone = torch.clone(io)
            io_clone[..., :2] = (io[..., :2] * 2. - 0.5 + self.grid)
            io_clone2 = torch.clone(io_clone)
            io_clone2[..., 2:4] = (io_clone[..., 2:4] * 2) ** 2 * self.anchor_wh
            #io[..., :4] *= self.stride
            io_clone3 = torch.clone(io_clone2)
            io_clone3[..., :4] = io_clone2[..., :4] * self.stride
            #io = p.clone()  # inference output
            #io[..., :2] = torch.sigmoid(io[..., :2]) + self.grid  # xy
            #io[..., 2:4] = torch.exp(io[..., 2:4]) * self.anchor_wh  # wh yolo method
            #io[..., :4] *= self.stride
            #torch.sigmoid_(io[..., 4:])
            return io_clone3.view(bs, -1, self.no), p  # view [1, 3, 13, 13, 85] as [1, 507, 85]
